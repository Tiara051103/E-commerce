...
def produk():
    ...
    if request.method == "POST":
        ...
        kategori_ids = [int(kid) for kid in kategori_ids]
        ...
        for kategori_id in kategori_ids:
            produk_kategori_repo.add(hasil.data.id, kategori_id)
...
def detail_produk(id):
    ...
    produk.brand = None
    if produk.brand_id:
        produk.brand = brand_repo.get_by_id(int(produk.brand_id))

    kategori_ids = produk_kategori_repo.get_kategori_ids_by_produk(id)
    produk.kategori_list = kategori_repo.get_by_ids(kategori_ids)
    ...
    if request.method == "POST":
        ...
        for kategori_id in request.form.getlist("kategori_ids"):
            if not kategori_id:  # 🔥 CEGAH ""
                continue
            produk_kategori_repo.add(id, int(kategori_id))

        flash("Produk berhasil diupdate", "success")
        return redirect(url_for("produk.daftar_produk", id=id))
